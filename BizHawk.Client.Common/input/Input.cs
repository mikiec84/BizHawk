...
namespace BizHawk.Client.Common
{
	public class Input
	{
		...
		private readonly ClientMainFormCommon MainForm;
		readonly Thread UpdateThread;

		private Input(ClientMainFormCommon mainForm)
		{
			MainForm = mainForm;
		...
		public static void Initialize(ClientMainFormCommon mainForm)
		{
			if (PlatformLinkedLibSingleton.CurrentOS == PlatformLinkedLibSingleton.DistinctOS.Windows)
			{
				KeyInput.Initialize(mainForm.Handle);
				IPCKeyInput.Initialize();
				GamePad.Initialize(mainForm.Handle);
		...
		void UpdateThreadProc()
		{
			while (true)
			{
				...
				lock (this)
				{
					...
					bool swallow = !MainForm.AllowInput(false);

					foreach (var ie in _NewEvents)
					{
						//events are swallowed in some cases:
						if (ie.LogicalButton.Alt && !MainForm.AllowInput(true))
						{ }
		...
		public string GetNextBindEvent()
		{
			...
			{
				if (InputEvents.Count == 0) return null;
				if (!MainForm.AllowInput(false)) return null;
